...
enum VarKindEnum
{    //变量的类别,分别表示变量是标志符变量，数组成员变量还是域成员变量
    IdV,
    ArrayMembV,
    FieldMembV
};
...
union Kind
{
    DeckEnum dec;           // 记录语法树节点的声明类型,当nodekind=DecK时有效，确保在语法树节点的不同类型之间没有冲突或混淆
...
{    //记录数组类型的属性。
    int low;               // 数组下界
    int up;                // 数组上界
    DeckEnum childType;    // 数组元素类型
};
struct ProcAttr
{    //记录过程的属性。
    ParamtEnum paramt;      // 记录过程的参数类型，取值Valparamtype,Varparamtype
};
struct ExpAttr
{    //记录表达式的属性。
