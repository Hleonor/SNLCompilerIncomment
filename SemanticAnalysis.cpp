...
void Inputsymb(string path)
{
    ...
    for (int i = 1; i < level + 1; i++)
    {
        ...
        while (t != nullptr)
        {
            ...
            if (t->attrIR.kind == typeKind)
            {
                ouput << t->idname << " " << t->attrIR.kind << " " << &t->attrIR << " " << t->attrIR.idtype << " "
                      << t->attrIR.More.VarAttr.level << endl;
            }
            else if (t->attrIR.kind == varKind)
            {
                if (t->attrIR.idtype != nullptr)
                {
                    ouput << t->idname << " " << t->attrIR.kind << " " << &t->attrIR << " " << t->attrIR.idtype << " "
                          ...
                          << t->attrIR.idtype->size << endl;
                }
                else
                { ouput << t->idname << " " << t->attrIR.kind << " " << &t->attrIR << " " << endl; }
            }
            else
            {
                ouput << t->idname << " " << t->attrIR.kind << " " << &t->attrIR << " " << t->attrIR.idtype << " "
                      << t->attrIR.More.ProcAttr.level << " " << t->attrIR.More.ProcAttr.param << endl;
            }
...
void Inputdestroysymb(string path)
{
    ...
    for (int i = 0; i < destroylevel; i++)
    {
        ...
        while (t != nullptr)
        {
            ...
            {
                ouput << t->idname << " " << t->attrIR.kind << " " << &t->attrIR << " " << t->attrIR.idtype << " "
                      << t->attrIR.More.VarAttr.level << " " << t->attrIR.idtype->kind << " " << t->attrIR.idtype->size
                      << endl;
            }
            else
            {
...
void bianli(TreeNode *t)
{
    ...
    if (t->nodekind == PheadK) // 程序头结点
    {
        return;
    }
    if (t->nodekind == TypeK) // 类型结点
    {
        TypeDecPart(t->child[0]);
        bianli(t->sibling);
        return;
    }
    if (t->nodekind == VarK) // 变量结点
    {
        VarDecList(t->child[0]);
        bianli(t->sibling);
        return;
    }
    if (t->nodekind == ProcDecK) // 过程节点
    {
        ProcDecPart(t);
        return;
    }
    if (t->nodekind == StmLK) // 语句序列结点
    {
        Body(t->child[0]);
        return;
    }
